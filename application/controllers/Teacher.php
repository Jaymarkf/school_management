...
class Teacher extends CI_Controller
{
    ...
    function manage_attendance_view($class_id = '' , $section_id = '' , $timestamp = '',$subject_id ='')
    {
        if($this->session->userdata('teacher_login')!=1)
            redirect(base_url() , 'refresh');
        $class_name = $this->db->get_where('class' , array(
...
//        ))->result_array();
    ...
    function grade_selector(){
        ...
        $data['subject_id'] = $this->input->post('subject_id');
        $data['year'] = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
        redirect(base_url().'index.php?teacher/manage_grade/'.$data['class_id'].'/'.$data['section_id'].'/'.$data['subject_id'].'/'.$data['semester_id'],'refresh');
    }
